...
export default function CustomersPage() {
  ...
  const loadCustomers = async () => {
    try {
      const data = await api.getLedgers(selectedCompany!.id, { is_customer: true });
      setCustomers(data);
    } catch (err) {
      console.error(err);
    } finally {
        ...
        <header className="header px-10 py-7 flex items-center justify-between">
          <div>
            <div className="flex items-center gap-4 mb-2">
              <h2 className="text-3xl font-black text-white">Customers</h2>
            ...
            <div className="glass-card table-glass rounded-3xl border border-white/10 overflow-hidden fade-in">
              {customers.length === 0 ? (
                <div className="p-24 text-center">
                  ...
                  <h3 className="text-2xl font-bold text-slate-200 mb-3">No Customers Yet</h3>
                  <p className="text-slate-400 text-lg mb-6">Create your first customer from the Ledgers page</p>
                  <button
                ...
                <table className="min-w-full divide-y divide-white/10">
                  ...
                  <tbody className="divide-y divide-white/5">
                    {customers.map((customer) => (
                      <tr key={customer.id} className="table-row hover:bg-purple-500/10 transition-all">
                        <td className="px-8 py-6 whitespace-nowrap">
                          <div className="text-lg font-semibold text-white">{customer.name}</div>
                        </td>
                        <td className="px-8 py-6 whitespace-nowrap">
                          <span className="text-slate-300">{customer.group_name}</span>
                        </td>
                        <td className="px-8 py-6 whitespace-nowrap">
                          <div className={`text-lg font-bold ${
                            customer.opening_balance_type === 'debit' ? 'text-blue-300' : 'text-emerald-300'
                          }`}>
                            ₹{Number(customer.opening_balance || 0).toFixed(2)} {customer.opening_balance_type?.toUpperCase()}
                          </div>
                        </td>
                        <td className="px-8 py-6 whitespace-nowrap">
