    ...
    <div className="erp-page-container flex flex-row">
      ...
      <main className="flex-1 flex flex-col overflow-hidden bg-[var(--erp-bg)]">
        <div className="erp-header relative overflow-hidden">
          ...
          <div>
            <h2 className="erp-title">Customers</h2>
          ...
          <table className="erp-table">
            ...
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td colSpan={5} className="text-center py-8">No customers found. Create one in Ledgers.</td>
                </tr>
              ) : (
                customers.map((customer, i) => (
                  <tr key={customer.id}>
                    <td className="text-center text-[var(--erp-text-muted)]">{i + 1}</td>
                    <td className="font-medium text-[var(--erp-teal)]">{customer.name}</td>
                    <td className="text-[var(--erp-text-muted)]">{customer.group_name}</td>
                    <td className="text-right font-medium">
                      {new Intl.NumberFormat('en-IN', { minimumFractionDigits: 2 }).format(Number(customer.opening_balance || 0))}
                      <span className="ml-1 text-xs opacity-70">
                        {customer.opening_balance_type === 'debit' ? 'Dr' : 'Cr'}
                      </span>
                    </td>
                    <td className="text-center">
                      <span className={customer.is_active ? 'erp-badge' : 'erp-badge erp-badge-danger'}>
