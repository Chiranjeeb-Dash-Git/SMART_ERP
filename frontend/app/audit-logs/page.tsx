...
export default function AuditLogsPage() {
  ...
  const loadData = async () => {
    try {
      const data = await api.getAuditLogs(selectedCompany!.id);
      setLogs(data);
    } catch (err) {
      console.error(err);
    } finally {
        ...
        <header className="header px-10 py-7 flex items-center justify-between">
          <div>
            <div className="flex items-center gap-4 mb-2">
              <h2 className="text-3xl font-black text-black">Audit Logs</h2>
        ...
        <div className="flex-1 overflow-auto p-10">
          {loading ? (
            <div className="flex flex-col items-center justify-center h-full text-[var(--erp-text-muted)]">
              ...
              <p className="text-xl font-semibold">Loading audit logs...</p>
            </div>
          ) : (
            <div className="erp-table-container rounded-3xl border border-gray-200 overflow-hidden fade-in">
              {logs.length === 0 ? (
                <div className="p-24 text-center">
                  ...
                  <h3 className="text-2xl font-bold text-black font-semibold mb-3">No Audit Logs Yet</h3>
                  <p className="text-[var(--erp-text-muted)] text-lg">Activity logs will appear here as you use the system.</p>
                </div>
              ) : (
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {logs.map((log) => (
                      <tr key={log.id} className="table-row hover:bg-gray-100 transition-all">
                        <td className="px-8 py-6 whitespace-nowrap">
                          <div className="text-lg font-semibold text-black">{log.user_name || 'System'}</div>
                        </td>
                        <td className="px-8 py-6 whitespace-nowrap">
                          <span className="text-[var(--erp-text-muted)]">{log.action}</span>
                        </td>
                        <td className="px-8 py-6 whitespace-nowrap">
                          <span className="text-[var(--erp-text-muted)]">{log.module || '-'}</span>
                        </td>
                        <td className="px-8 py-6 whitespace-nowrap">
